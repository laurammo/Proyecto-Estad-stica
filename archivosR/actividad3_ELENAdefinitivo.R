...
# OJO!! Les he cambiado el nombre a PC1 y PC2 NO TOCAR!! sino falla la tabla.
columnas <- data_act3 %>% select(c(1:26))
tercilescolumnas <- alimentos_nutrientes %>% select(c("PC1", "PC2"))
# Junto en un df nuevo todas las columnas
df_tabla <- cbind(columnas, tercilescolumnas)

# Juntamos en un nuevo df (df_tablaPC) las columnas con los terciles.
df_tablaPC <- df_tabla %>%
